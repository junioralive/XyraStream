...
const ContinueWatchingCard = ({ data, hidden }) => {
  ...
  return (
    <Link
      ...
      href={`/watch/${data?.id}?media_type=${data?.media_type}&se=${data?.season?.replace("Season ", "")}&ep=${data?.episode}`}
    >
      <Image
        src={data?.thumbnail}
        alt="thumbnail"
      ...
      <div className="w-full h-20 absolute flex flex-col justify-between bottom-0 after:content-[''] after:w-full after:h-28 after:absolute after:bottom-0 after:bg-[linear-gradient(360deg,#060602,#00000069,transparent)]">
        <div className="flex flex-col mx-3 justify-between gap-3 z-10">
          <div className="flex items-center justify-between w-full">
            <div>
              <div className="text-white w-full text-wrap break-words overflow-hidden text-ellipsis line-clamp-1 font-['poppins'] text-lg cursor-pointer hover:text-slate-200">
                {data?.title}
              </div>
              {data?.media_type !== "movie" && (
                <div className="text-[#ffffff8a] font-['poppins'] text-[14px]">
                  S{data?.season?.replace("Season ", "")} : EP {data?.episode}
                </div>
              )}
            </div>
